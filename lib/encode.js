...
(function (write, fromString) {
	...
	var ext = function (encoder, value) {
		...
		byteLength <= 0xff ? (write.int8(encoder, 0xc7, byteLength), write.type(value.etype)) :
		byteLength <= 0xffff ? (write.int16(encoder, 0xc8, byteLength), write.type(value.etype)) :
		(write.int32(encoder, 0xc9, byteLength), write.type(value.etype));
	...
	var map = function (encoder, value) {
		...
		length <= 0xffff ? write.type(encoder, 0xde, length) :
		write.type(encoder, 0xdf, length);
		
		for (var i=0; i<length; ++i) {
			encode(encoder, keys[i]);
			encode(encoder, value[keys[i]]);
		}
	};
